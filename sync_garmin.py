...
for i in range(DAYS):
    ...
    try:
        ...
        def row(metric, value, raw_json=None):
            return {"user_id": USER_ID, "local_date": ds, "metric": metric, "value": value, "source": "garmin", "raw": raw_json}

        rows.append(row("sleep_score", score, raw))
        if total_min > 0:
            rows.append(row("sleep_minutes", total_min))
        if deep_sec > 0:
            rows.append(row("sleep_deep_minutes", round(deep_sec / 60)))
        if light_sec > 0:
            rows.append(row("sleep_light_minutes", round(light_sec / 60)))
        if rem_sec > 0:
            rows.append(row("sleep_rem_minutes", round(rem_sec / 60)))
        if awake_sec > 0:
            rows.append(row("sleep_awake_minutes", round(awake_sec / 60)))
